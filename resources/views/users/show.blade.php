        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            @if(request()->has('rfid_card_id'))
                @if(auth()->user()->role == "Guard")
                    <!-- Entry Form -->
                    <div class="grid grid-cols-1 md:grid-cols-1 gap-2 bg-white rounded-md pb-5 pt-2 px-5 shadow-xl sm:rounded-lg mb-6">
                        <form action="{{ route('entries.store') }}" method="POST" class="">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                                ...
                                <div>
                                    ...
                                    <select name="vehicle_id" id="vehicle_id" class="block w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm">
                                        <option value="NULL">None</option>
                                        @foreach($user->vehicles as $vehicle)
                                            @if($user->vehicles->count() == 1)
                                                <option value="{{ $vehicle->id }}" selected>{{ $vehicle->vehicle_no }}</option>
                                            @else
                ...
                <div
                    ...
                    <div class="p-6 sm:p-8">
                        <h3 class="text-xl font-bold text-white mb-6 flex items-center">
                            ...
                            <div class="mt-6 flex justify-right">
                                <a href="{{ route('family-members.rfid_card_create_via_user', $user->id) }}" class="bg-white text-purple-600 font-bold text-sm py-2 px-4 rounded-lg shadow-md hover:bg-purple-700 hover:text-white transition duration-300">
                                    + Add Member
                                </a>
                            </div>
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg mb-4">
                ...
                <div class="p-1">
                    @if ($user->familyMembers->count() > 0)
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 animate-cards">
                            @foreach ($user->familyMembers as $card)
                                <div class="group relative bg-white rounded-lg border border-gray-200 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-md animate-cardEntrance"
                                     style="animation-delay: {{ $loop->index * 100 }}ms">
                                    <div
                                        class="absolute top-0 left-0 w-full h-0.5 bg-gradient-to-r transition-colors duration-300
            ...
            <a href="{{ route('vehicles.vehicles_create_via_user', $user->id) }}"></a>
            <div
                ...
                <div class="border-b border-gray-200 dark:border-gray-700">
                    <div class="p-6 flex justify-between items-center">
                        ...
                        <button
                            onclick="Livewire.emit('openModal', 'add-vehicle', {{ json_encode(['userId' => $user->id]) }})"
                            ...
                            @foreach ($user->vehicles as $vehicle)
                                <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-6 relative group">
                                    <div
                                        ...
                                        <button
                                            onclick="Livewire.emit('openModal', 'edit-vehicle', {{ json_encode(['vehicleId' => $vehicle->id]) }})"
                                    ...
                                    <div class="space-y-2">
                                        <div class="flex justify-between">
                                            ...
                                            <span
                                                class="text-sm text-gray-900 dark:text-gray-100">{{ $vehicle->color }}</span>
                                        </div>
                                        <div class="flex justify-between">
                                            <span class="text-sm text-gray-500 dark:text-gray-400">Year</span>
                                            <span
                                                class="text-sm text-gray-900 dark:text-gray-100">{{ $vehicle->year }}</span>
                                        </div>
                                    </div>
                                </div>
                            @endforeach
